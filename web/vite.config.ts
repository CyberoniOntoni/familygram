...
export default defineConfig(({ mode }): UserConfig => {
  ...
  const csp = buildCsp(appEnv, FAMILYGRAM_SELF_HOSTED === '1');
  ...
  return {
    ...
    define: {
      ...
      'process.env.FAMILYGRAM_SELF_HOSTED': JSON.stringify(FAMILYGRAM_SELF_HOSTED || '0'),
    },
    resolve: {
      tsconfigPaths: true,
      alias: [
...
function buildCsp(appEnv: string, isFamilyGram = false) {
  return `
  default-src 'self';
  connect-src 'self' wss://*.web.telegram.org blob: http: https: ${appEnv === 'development' || isFamilyGram ? 'wss: ws:' : ''} ${appEnv === 'development' ? 'ipc:' : ''};
