...
cmds = [
    ...
    ('ENV WebRTC/DC', 'grep -E "WebRtc|DcOptions|Servers|FixedVerify|Coturn|Turn|5348|20443|30444|ApiId" /opt/testgram/docker/compose/.env 2>/dev/null | head -40'),
    ...
    ('MONGO users', 'cd /opt/testgram/docker/compose && docker compose exec -T mongodb mongosh tg --quiet --eval "db.getCollection(\\"eventflow-userreadmodel\\").find({}, {UserId:1,FirstName:1,PhoneNumber:1}).toArray()"'),
    ('MONGO messages sample', 'cd /opt/testgram/docker/compose && docker compose exec -T mongodb mongosh tg --quiet --eval "db.getCollectionNames().filter(c=>c.toLowerCase().includes(\\"message\\")).join(\\", \\")"'),
    ('MESSAGE count', 'cd /opt/testgram/docker/compose && docker compose exec -T mongodb mongosh tg --quiet --eval "c=db.getCollectionNames().find(n=>n.includes(\\"message\\")); c?db.getCollection(c).countDocuments({}):0"'),
