...
import {
  APP_CODE_NAME,
  DEBUG, DEBUG_GRAMJS, IS_FAMILYGRAM, IS_TEST, LANG_PACK, TELEGRAM_API_HASH, TELEGRAM_API_ID, UPLOAD_WORKERS,
} from '../../../config';
import { applyFamilyGramTlCompat } from '../../../util/familygramTlCompat';
...
export async function init(initialArgs: ApiInitialArgs, onConnected?: NoneToVoidFunction) {
  ...
  if (IS_FAMILYGRAM) {
    applyFamilyGramTlCompat();
    if (webTransportHost && webTransportPort) {
  ...
  try {
    ...
    try {
      ...
      const connectTimeoutPromise = IS_FAMILYGRAM
        ? new Promise<never>((_, reject) => {
          connectTimeoutId = setTimeout(() => {
            reject(new Error('FamilyGram connection timeout'));
          }, 45000);
        })
        : undefined;
      ...
      if (connectTimeoutPromise) {
        await Promise.race([startPromise, connectTimeoutPromise]);
      } else {
        await startPromise;
      }
    } catch (err: any) {
...
export function uploadFile(file: File, onProgress?: ApiOnProgress) {
  return client.uploadFile({ file, onProgress, workers: IS_FAMILYGRAM ? 1 : UPLOAD_WORKERS });
}
