...
import { DEBUG, FORCE_FALLBACK_LANG, IS_FAMILYGRAM, LANG_PACK } from '../../config';
...
import readFamilyGramOverrides from '../data/readFamilyGramOverrides';
...
let familyGramOverrides: Record<string, LangPackStringValue> | undefined;
let familyGramOverridesPromise: Promise<void> | undefined;

async function loadFamilyGramOverrides() {
  if (!IS_FAMILYGRAM || familyGramOverrides || familyGramOverridesPromise) return;
  familyGramOverridesPromise = readFamilyGramOverrides().then((overrides) => {
    familyGramOverrides = overrides;
    if (fallbackLangPack) {
      fallbackLangPack = {
        ...fallbackLangPack,
        strings: {
          ...fallbackLangPack.strings,
          ...overrides,
        },
      };
    }
  ...
  await familyGramOverridesPromise;
}
